...
;(() => {
  ...
  const SCREENS = {
    lotteryResults: (listingId) => [
      { path: `/lottery-results?listing_id=${listingId}`, redact: 'allow', allow: PREFERENCE_ALLOW },
  ...
  const DENY_KEY = /name|email|phone|address|street|city|zip|postal|dob|date_of_birth|birth|ssn|social|first|last|middle|contact|applicant|member|agent|alternate|guardian/i
  ...
  const scrubValue = (value) =>
    typeof value === 'string'
      ? VALUE_PATTERNS.reduce((acc, [pattern, replacement]) => acc.replace(pattern, replacement), value)
      : value
  ...
  window.captureFixtures = async (listingId, { screen = 'lotteryResults', dryRun = false } = {}) => {
    ...
    console.log('[fixtures] downloaded. Unpack with: bin/unpack-fixtures ~/Downloads/lap-fixtures-' + listingId + '.json')
    return bundle
  }
